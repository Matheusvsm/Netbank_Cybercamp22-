DROP FUNCTION public.getauthorizationtokenbytaxid(character varying);

CREATE OR REPLACE FUNCTION public.getauthorizationtokenbytaxid(
	"paramTaxId" character varying)
    RETURNS TABLE("AuthorizationTokenId" bigint, "UserId" bigint, "AccountId" bigint, "TaxId" character varying, "Code" character varying, "ExpirationDate" timestamp without time zone, "Salt" character varying, "Status" integer, "CreationDate" timestamp without time zone, "UpdateDate" timestamp without time zone, "CreationUserId" bigint, "UpdateUserId" bigint, "DeletionDate" timestamp without time zone, "ValidateAttempts" integer) 
    LANGUAGE 'sql'
    COST 100
